...
- [ ] 1.1 Create `packages/websearch-mcp-server/` directory structure: `src/`, plus `package.json` and `tsconfig.json` mirroring the conventions in `packages/mcp-server/`
- [ ] 1.2 Write `packages/websearch-mcp-server/package.json` with `name: "@gsd/websearch-mcp-server"`, `type: "module"`, `bin: { "gsd-websearch-mcp-server": "./dist/cli.js" }`, `exports` map, and `dependencies: { @modelcontextprotocol/sdk, cheerio, zod }`
...
- [ ] 1.4 Register the new package in the monorepo's `pnpm-workspace.yaml`
- [ ] 1.5 Run `pnpm install` from root to install cheerio and register the new workspace package

## 2. Implement Rate Limiter

- [ ] 2.1 Create `packages/websearch-mcp-server/src/rate-limiter.ts` — export `RateLimiter` class with:
...
- [ ] 2.2 Write unit tests for `RateLimiter` covering: single request allowed, rapid requests rejected, window expiry, approaching-limit threshold, configurable RPM, and memory bounds

## 3. Implement DuckDuckGo Search Client

- [ ] 3.1 Create `packages/websearch-mcp-server/src/search-duckduckgo.ts` — export `searchDuckDuckGo(query, options)` function:
...
- [ ] 3.2 Write unit tests for DuckDuckGo search client covering: search returns results, empty query handling, site filter injection, freshness parameter passthrough, CAPTCHA fallback detection logic, network error propagation

## 4. Implement MCP Server Core

- [ ] 4.1 Create `packages/websearch-mcp-server/src/server.ts` — MCP server setup using `@modelcontextprotocol/sdk` `McpServer`:
...
- [ ] 4.2 Create `packages/websearch-mcp-server/src/cli.ts` — CLI entry point:
  - Instantiate `Server` with `StdioServerTransport`
  - Handle process signals for clean shutdown
- [ ] 4.3 Create `packages/websearch-mcp-server/src/index.ts` — re-export all public types and classes

## 5. Create Auto-Registration Extension

- [ ] 5.1 Create `src/resources/extensions/websearch-mcp/extension-manifest.json` with `{ id: "websearch-mcp", name: "WebSearch MCP", tier: "bundled" }`
- [ ] 5.2 Create `src/resources/extensions/websearch-mcp/index.ts` — extension default export:
...
- [ ] 6.1 Add `packages/websearch-mcp-server` to the `build:core` script chain in root `package.json`
- [ ] 6.2 Verify the package compiles: run `npm run build:core` and confirm `dist/` output exists for the new package
- [ ] 6.3 Add basic smoke test: `packages/websearch-mcp-server/src/__tests__/server.test.ts` — tests that tools are registered with the correct names and schemas

## 7. Update Documentation

- [ ] 7.1 Add a "WebSearch MCP Server" section to `docs/user-docs/providers.md` explaining:
...
- [ ] 7.2 Add the same section to `docs/zh-CN/user-docs/providers.md`
- [ ] 7.3 Add the same section to `gitbook/configuration/providers.md`
